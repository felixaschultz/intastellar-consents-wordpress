...
/**
 * Plugin Name: Intastellar Consents – GDPR Cookie Banner & Google Consent Mode
...
add_action('plugins_loaded', function () {
    ...
    include_once plugin_dir_path(__FILE__) . 'includes/intastellar-cookie-table.php';
}, 1);
...
function intastellar_load_textdomain()
{
    ...
    $domain     = 'intastellar-gdpr-cookie-banner';
...
add_filter('load_textdomain_mofile', function ($mofile, $domain) {
    if ($domain !== 'intastellar-gdpr-cookie-banner') {
        return $mofile;
    }
    ...
    foreach (array_unique($locales) as $locale) {
        $plugin_mo = $lang_dir . 'intastellar-gdpr-cookie-banner-' . $locale . '.mo';
        ...
        if (count($parts) >= 2) {
            $base = $parts[0] . '_' . $parts[1];
            if ($base !== $locale) {
                $plugin_mo = $lang_dir . 'intastellar-gdpr-cookie-banner-' . $base . '.mo';
                if (file_exists($plugin_mo)) {
                    return $plugin_mo;
                }
            }
